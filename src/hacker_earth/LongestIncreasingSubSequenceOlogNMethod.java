...
public class LongestIncreasingSubSequenceOlogNMethod {
	...
	public int longestIncreasingSubSequence(int input[]) {
		...
		for (int i = 1; i < input.length; i++) {
			if (input[T[0]] > input[i]) { // if input[i] is less than 0th value
											// of T then replace it there.
				T[0] = i;
			} else if (input[T[len]] < input[i]) { // if input[i] is greater
													// than last value of T then
													// append it in T
				len++;
				T[len] = i;
				R[T[len]] = T[len - 1];
			} else { // do a binary search to find ceiling of input[i] and put
						// it there.
	...
	static int[] BS(int[] a) {
		...
		for (int i = 0; i < a.length; i++) {
			...
			sol[i] = lis.longestIncreasingSubSequence(a);
		}
		return sol;
	}
}
